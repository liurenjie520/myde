...
year = datetime.datetime.today().year
year=str(year)
url = "http://www.51wnl.com/YJData/"+year+".json"
url2="http://www.51wnl.com/moreLumarData/"+year+".json"
...
def getYJData():
    ...
    with open('yj'+year + '.json', 'w', encoding='utf-8') as f:
        f.write(json.dumps(content_dict, ensure_ascii=False))
        print("ok")


def getmoreLumarData():
    resp = requests.get(url2, timeout=TIMEOUT)
    content_dict = resp.json()
    year = datetime.datetime.today().year
    year = str(year)

    with open('other'+year + '.json', 'w', encoding='utf-8') as f:
        f.write(json.dumps(content_dict, ensure_ascii=False))
        print("ok")
